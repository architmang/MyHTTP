...
void log_access(int client_sock, char* method, char* url) {
    char buffer[BUFSIZ];

    time_t timey;
    struct tm* ti;
    time(&timey);
    ti = localtime ( &timey );                                             // reports local system time
    ...
    // format log entry
    strftime(buffer, sizeof(buffer), "%d%m%y:%H%M%S", asctime(ti));
    fprintf(stdout, "%s:%s:%s:%d:%s:%s\n", buffer, client_ip, client_port, method, url);

    // write log entry to file
    FILE* logfile = fopen("AccessLog.txt", "a");
    if (logfile != NULL) {
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        // if (fork() == 0)
        {
            ...
            // log_access(new_socket, method, host);
            if (strcmp(method, "PUT") == 0 || strcmp(method, "put") == 0)
            {
